...
export async function getUserId() {
  ...
  if (sessionToken) {
    try {
      ...
      console.error('Invalid JWT session token:', error);
      // Fallback to checking the other cookie instead of immediately destroying
    }
  }

  // 2. Try custom auth_session cookie (Handle cases where only this is present)
  ...
  if (authSessionCookie) {
    try {
      ...
      if (sessionData.phone && sessionData.token) {
        // Find user by phone and token to get the actual ID
      ...
      console.error('Failed to parse auth_session cookie in getUserId:', e);
    }
  }
