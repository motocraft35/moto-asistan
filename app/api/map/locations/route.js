...
export async function GET(request) {
    try {
        ...
        // 3. Fetch all with dynamic rolling 24h owner info
        let query = `
            WITH WinnerClan AS (
                SELECT locationId, clanId, COUNT(*) as cnt
                FROM location_checkins
                WHERE timestamp > datetime('now', '-24 hours') AND clanId IS NOT NULL
                GROUP BY locationId, clanId
            ),
            MaxWinner AS (
                SELECT locationId, clanId, MAX(cnt) as max_cnt
                FROM WinnerClan
                GROUP BY locationId
            )
            SELECT 
                ml.*, 
                c.name as ownerName, 
                c.logoUrl as ownerLogo, 
                c.flagId,
                c.color as ownerColor,
                COALESCE(mw.max_cnt, 0) as checkInCount
            FROM map_locations ml
            LEFT JOIN MaxWinner mw ON ml.id = mw.locationId
            LEFT JOIN clans c ON mw.clanId = c.id
        `;
        let args = [];
