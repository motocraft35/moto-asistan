...
export async function PATCH(request) {
    try {
        ...
        const body = await request.json();
        const { destLatitude, destLongitude, destName, partyId } = body;

        // Verify user is the leader of this party
        const partyResult = await db.execute({
            sql: `SELECT leaderId FROM parties WHERE id = ?`,
            args: [partyId]
        });

        if (partyResult.rows.length === 0) return Response.json({ error: 'Party not found' }, { status: 404 });
        if (partyResult.rows[0].leaderId !== userId) return Response.json({ error: 'Only the leader can sync navigation' }, { status: 403 });

        await db.execute({
            sql: `UPDATE parties SET destLatitude = ?, destLongitude = ?, destName = ? WHERE id = ?`,
            args: [destLatitude, destLongitude, destName, partyId]
        });

        return Response.json({ success: true, message: 'Neural Sync Established' });
    } catch (error) {
